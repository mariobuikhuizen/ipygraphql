...
LONG_DESCRIPTION = 'Graphql widgets for Jupyter'


def get_data_files():
    tgz = 'jupyter-graphql-' + version_ns['__version__'] + '.tgz'
    return [
        ('share/jupyter/nbextensions/jupyter-graphql', glob.glob('ipygraphql/static/*')),
        ('etc/jupyter/nbconfig/notebook.d', ['jupyter-graphql.json']),
...
class NPM(Command):
    ...
    targets = [
        os.path.join(here, 'ipygraphql', 'static', 'extension.js'),
        os.path.join(here, 'ipygraphql', 'static', 'index.js')
    ]
...
version_ns = {}
with open(os.path.join(here, 'ipygraphql', '_version.py')) as f:
